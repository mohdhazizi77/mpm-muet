        ...
        $('#dt-auditLog').DataTable({
            ...
            columns: [
                ...
                },
                {
                    ...
                    orderable: true,
                    render: function(data, type, row, meta) {
                        console.log(row.user_name, row);

                        return `<p>${row.user_name}</p><p>ID: ${data}</p>`;
                    }
                },
                {
                    data: "action",
                    orderable: true,
                    render: function(data, type, row, meta) {
                        return `<p>${data}</p><p>${row.created_date}</p>`;
                    }
                },
                {
                    data: "data",
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        let html = `<p>${data.table} ID:${data.id}</p>`;

                        if (data.data?.old) {
                            html += '<p class="pt-3 fw-bold">Old</p><ul>';
                            Object.entries(data.data.old).forEach(([key, value]) => {
                                html += `<li>${key}: ${value}</li>`;
                            });
                            html += '</ul>';
                        ...
                        if (data.data?.new) {
                            html += '<p class="pt-3 fw-bold">New</p><ul>';
                            Object.entries(data.data.new).forEach(([key, value]) => {
                                html += `<li>${key}: ${value}</li>`;
                            });
                            html += '</ul>';
