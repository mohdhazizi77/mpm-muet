    ...
    if ($("#listCertTable").length > 0) {
        ...
        var table = $('#listCertTable').DataTable({
            ...
            ajax: {
                ...
                "data": function (d) {
                    d._token = $('meta[name="csrf-token"]').attr('content')
                    d.candidate_id = $('#candidate_id').val()
                }
            },
            columns: [
                // {
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        var buttonPrintPDF = '';
                        var buttonUpdateResult = '';

                        buttonUpdateResult =
                                        '<button type="button" class="btn btn btn-soft-secondary fs-12 waves-effect text-black d-block d-md-inline-block mb-2 mx-md-2" id="show_edit_modal" data-id="' + data + '">Update</button>'

                        // buttonPrintPDF =
                        //                 '<a href="/candidate/view-result/' + data + '" data-id=' + data + ' class="btn btn btn-soft-secondary fs-12 waves-effect text-black d-block d-md-inline-block mb-2 mx-md-2 printPdfButton modalVerify">' +
                        //                 '<i class="ri-printer-line label-icon align-middle fs-12 me-2"></i>' +
                        //                 'PRINT PDF' +
                        //                 '</a>';

                        buttonPrintPDF =
                                        '<a href="' + row.downloadUrl + '" target="_blank" class="btn btn btn-soft-secondary fs-12 waves-effect text-black d-block d-md-inline-block mb-2 mx-md-2">' +
                                        '<i class="ri-printer-line label-icon align-middle fs-12 me-2"></i>' +
                                        'PRINT PDF' +
                                        '</a>';
                        ...
                        // return buttonPrintPDF + buttonPrintMPM + buttonCheckCert;
                        return buttonUpdateResult + buttonPrintPDF;
                    },
                    responsivePriority: 5
                }
            ],
        ...
        $(document).on('click', '#submit', function (e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: '/admin/manage-candidate/updateCert/' + id,
                        method: 'POST',
                        data: formData,
                        processData: false,
                        contentType: false,
                            ...
                            if (response.success) {
                                ...
                                Swal.fire({
                                    ...
                                    if (result.value) {
                                        ...
                                        // Redraw the DataTable
                                        $('#listCertTable').DataTable().ajax.reload();
                                    }
                                });
                            } else {
                                Swal.fire({
