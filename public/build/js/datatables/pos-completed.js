    ...
    if ($('#dt-pos').length > 0) {
        var dt = $('#dt-pos').DataTable({
            responsive: true, processing: true, serverSide: false, ajax: {
                "url": "./pos-completed/ajax", "type": "POST", "data": function (data) {
                },
            }, columns: [{
                ...
                data: 'status', name: 'status', class: 'text-center', orderable: false, searchable: false, render(data, type, row) {
                    let btn = '';
                    btn += data == 'COMPLETED' ? '<button type="button" class="btn btn-info btn-icon waves-effect waves-light me-2" data-bs-toggle="modal" data-bs-target=".modalUpdatePos"><i class="ri-information-line fs-22"></i></button>' : '';
